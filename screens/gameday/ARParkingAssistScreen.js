...
export default function ARParkingAssistScreen({ navigation }) {
  ...
  const isFinalStep = parkingAssistSession.stepIndex === parkingAssistSteps.length - 1;
  const progressLabel = `${parkingAssistSession.stepIndex + 1} of ${parkingAssistSteps.length}`;
  ...
  const permissionState = useMemo(() => {
    if (cameraError) {
      return {
        ...
        detail: 'Use a physical device for live parking preview, or drop back to the lot map for the same route.',
        actionLabel: 'Use lot map',
    ...
    if (requestingPermission || permission == null) {
      return {
        title: 'Opening camera assist',
        detail: 'We are preparing the live view for your final approach into Row G and Spot G-142.',
        actionLabel: 'Preparing',
        onAction: () => {},
        secondaryLabel: 'Use lot map',
    ...
    if (permission?.canAskAgain) {
      return {
        title: 'Allow camera for live guidance',
        detail: 'The rear camera is used only during this parking assist so the final approach feels immediate and spatial.',
    ...
    return {
      ...
      detail: 'Turn camera access back on in system settings to use the live assist. Until then, the map route is the clean fallback.',
      actionLabel: 'Use lot map',
      onAction: handleMapFallback,
      secondaryLabel: 'Reset assist',
      onSecondaryAction: resetParkingAssist,
      icon: Map,
    };
  }, [cameraError, handleMapFallback, permission, requestingPermission, resetParkingAssist]);
          ...
          <View style={styles.headerCenter}>
            <View style={styles.headerChipRow}>
              <View style={styles.headerChip}>
                <Camera size={13} color={COLORS.maize} />
                <Text style={styles.headerChipText}>CAMERA ASSIST</Text>
              </View>
              <View style={styles.headerChipMuted}>
                <Text style={styles.headerChipMutedText}>{user.parking.lot}</Text>
              </View>
            </View>
            <Text style={styles.headerTitle}>Spot {user.parking.spot}</Text>
          </View>
            ...
            <View style={styles.bottomSheetWrap}>
              ...
              {parkingAssistSession.status === 'complete' ? (
                <>
                  ...
                  <TouchableOpacity style={styles.secondaryButton} activeOpacity={0.88} onPress={resetParkingAssist}>
                    <RotateCcw size={15} color={COLORS.textSecondary} />
                    <Text style={styles.secondaryButtonText}>Run camera assist again</Text>
                  </TouchableOpacity>
                </>
              ) : (
                <>
                  <View style={styles.sheetHeaderRow}>
                    <View>
                      <Text style={styles.sheetEyebrow}>FINAL APPROACH</Text>
                  ...
                  <View style={styles.progressRail}>
                    {parkingAssistSteps.map((step, index) => (
                      <View
                        key={step.id}
                        style={[
                          styles.progressPill,
                          index <= parkingAssistSession.stepIndex && styles.progressPillActive,
                        ]}
                      />
                    ))}
                  </View>
                  ...
                  <Text style={styles.sheetCaption}>Tap once after each lane change so guidance stays aligned.</Text>

                  <TouchableOpacity style={styles.primaryButton} activeOpacity={0.9} onPress={handlePrimaryAction}>
                    <Text style={styles.primaryButtonText}>{isFinalStep ? `PARKED IN ${user.parking.spot}` : 'DONE'}</Text>
                    <ChevronRight size={18} color={COLORS.blue} />
                  </TouchableOpacity>
                  ...
                  <View style={styles.sheetSecondaryRow}>
                    <TouchableOpacity style={styles.secondaryButton} activeOpacity={0.88} onPress={handleMapFallback}>
                      ...
                      <Text style={styles.secondaryButtonText}>Lot map</Text>
                    </TouchableOpacity>
